...
void UpdatePad(void)
{
	...
	for (i = 0; i < padCount; i++)
	{
		...
		switch (dijs.rgdwPOV[0])//十字キー
		{
		case 0: //上

			padState[i] |= BUTTON_CROSS_UP;
			break;

		case 9000: //右

			padState[i] |= BUTTON_CROSS_RIGHT;
			break;

		case 18000: //下

			padState[i] |= BUTTON_CROSS_DOWN;
			break;

		case 27000: //左

			padState[i] |= BUTTON_CROSS_LEFT;
			break;
		}


		if (dijs.lRz < -50)					padState[i] |= BUTTON_RIGHT_UP;
		//* x-axis (right)
		if (dijs.lRz > 50)					padState[i] |= BUTTON_RIGHT_DOWN;
		//* y-axis (forward)
		if (dijs.lZ < -50)					padState[i] |= BUTTON_RIGHT_LEFT;
		//* y-axis (backward)									  
		if (dijs.lZ > 50)					padState[i] |= BUTTON_RIGHT_RIGHT;
		...
		//* Ｘボタン
		if (dijs.rgbButtons[1] & 0x80)	padState[i] |= BUTTON_X;
		//* Ｃボタン
		if (dijs.rgbButtons[2] & 0x80)	padState[i] |= BUTTON_O;
		//* ▲ボタン
		if (dijs.rgbButtons[3] & 0x80)	padState[i] |= BUTTON_TRI;
		//* L1ボタン
		...
		//* SHERAボタン
		if (dijs.rgbButtons[8] & 0x80)	padState[i] |= BUTTON_SH;
		//* OPTIONSボタン
		if (dijs.rgbButtons[9] & 0x80)	padState[i] |= BUTTON_OP;
		//* Ｌ3ボタン
