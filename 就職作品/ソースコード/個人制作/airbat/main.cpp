...
#define WINDOW_NAME		"AIRBAT"		// ウインドウのキャプション名
...
HRESULT Init(HWND hWnd, BOOL bWindow)
{
	...
	//フォグの設定
	FLOAT StartPos = 1500;  //開始位置
	FLOAT EndPos = 10000; //終了位置
	g_pD3DDevice->SetRenderState(D3DRS_FOGENABLE, TRUE); //フォグ：ON
	g_pD3DDevice->SetRenderState(D3DRS_FOGCOLOR, D3DCOLOR_ARGB(255, 255, 255, 255)); //白色で不透明
