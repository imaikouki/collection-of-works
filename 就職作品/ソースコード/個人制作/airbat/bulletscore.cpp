...
#define	SCORE_POS_X			(SCREEN_WIDTH-640)	// スコアの表示基準位置Ｘ座標
#define	SCORE_POS_Y			(400.0f)																	// スコアの表示基準位置Ｙ座標

//*****************************************************************************
// プロトタイプ宣言
//*****************************************************************************
HRESULT MakeVertexBulletScore(LPDIRECT3DDEVICE9 pDevice);
void SetTextureBulletScore(int idx, int number);

//*****************************************************************************
// グローバル変数宣言
//*****************************************************************************
LPDIRECT3DTEXTURE9		g_pD3DTextureBulletScore[1] = {};	// テクスチャへのポインタ
LPDIRECT3DVERTEXBUFFER9 g_pD3DVtxBuffBulletScore = NULL;		// 頂点バッファインターフェースへのポインタ

D3DXVECTOR3				g_posBulletScore;						// 位置
D3DXVECTOR3				g_rotBulletScore;						// 向き

int						g_bulletscore;						// スコア
...
HRESULT InitBulletScore(void)
{
	...
	g_posBulletScore = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	g_rotBulletScore = D3DXVECTOR3(0.0f, 0.0f, 0.0f);

	// スコアの初期化
	g_bulletscore = 100;

	// 頂点情報の作成
	MakeVertexBulletScore(pDevice);

	// テクスチャの読み込み
	D3DXCreateTextureFromFile(pDevice,					// デバイスへのポインタ
								TEXTURE_SCORE,			// ファイルの名前
								&g_pD3DTextureBulletScore[0]);	// 読み込むメモリー

	//D3DXCreateTextureFromFile(pDevice,					// デバイスへのポインタ
	//							TEXTURE_FRAME_SCORE,		// ファイルの名前
	//							&g_pD3DTextureBulletScore[1]);	// 読み込むメモリー
...
{
	if(g_pD3DTextureBulletScore[0] != NULL)
	{// テクスチャの開放
		g_pD3DTextureBulletScore[0]->Release();
		g_pD3DTextureBulletScore[0] = NULL;
	}
	//if(g_pD3DTextureBulletScore[1] != NULL)
	//{// テクスチャの開放
	//	g_pD3DTextureBulletScore[1]->Release();
	//	g_pD3DTextureBulletScore[1] = NULL;
	//}

	if(g_pD3DVtxBuffBulletScore != NULL)
	{// 頂点バッファの開放
		g_pD3DVtxBuffBulletScore->Release();
		g_pD3DVtxBuffBulletScore = NULL;
	}
}
...
void UpdateBulletScore(void)
{
	for(int nCntPlace = 0; nCntPlace < NUM_PLACE; nCntPlace++)
	{
		...
		number = (g_bulletscore % (int)(powf(10.0f, (float)(NUM_PLACE - nCntPlace)))) / (int)(powf(10.0f, (float)(NUM_PLACE - nCntPlace - 1)));
		SetTextureBulletScore(nCntPlace, number);
	}
}
...
void DrawBulletScore(void)
{
    ...
    pDevice->SetStreamSource(0, g_pD3DVtxBuffBulletScore, 0, sizeof(VERTEX_2D));

	// 頂点フォーマットの設定
	pDevice->SetFVF(FVF_VERTEX_2D);

	// テクスチャの設定
	pDevice->SetTexture(0, g_pD3DTextureBulletScore[0]);
	...
	//// テクスチャの設定
	//pDevice->SetTexture(0, g_pD3DTextureBulletScore[1]);
...
HRESULT MakeVertexBulletScore(LPDIRECT3DDEVICE9 pDevice)
{
    ...
    if( FAILED( pDevice->CreateVertexBuffer(sizeof(VERTEX_2D) * (NUM_VERTEX * NUM_PLACE + 4),	// 頂点データ用に確保するバッファサイズ(バイト単位)
												...
												&g_pD3DVtxBuffBulletScore,							// 頂点バッファインターフェースへのポインタ
												NULL)))											// NULLに設定
	{
        return E_FAIL;
	}
	...
	{//頂点バッファの中身を埋める
		...
		g_pD3DVtxBuffBulletScore->Lock(0, 0, (void**)&pVtx, 0);
		...
		// 頂点データをアンロックする
		g_pD3DVtxBuffBulletScore->Unlock();
	}
...
void ChangeBulletScore(int value)
{
	...
	else if(g_bulletscore >= (int)(powf(10.0f, (float)(NUM_PLACE + 1))))
	{
		g_bulletscore = (int)(powf(10.0f, (float)(NUM_PLACE + 1))) - 1;
	}
}
